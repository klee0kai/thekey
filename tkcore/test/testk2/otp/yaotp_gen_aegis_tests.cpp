...
TEST(YaOtpGenAegis, Get2Test) {
    ...
    auto yaotpList = parseOtpUri(yaotpUri, [&](const OtpInfo &) {
        return Result<string>{.result = "7586"};
    });
...
TEST(YaOtpGenAegis, Get4Test) {
    ...
    auto yaotpList = parseOtpUri(yaotpUri, [&](const OtpInfo &) {
        return Result<string>{.result = "5210481216086702"};
    });
